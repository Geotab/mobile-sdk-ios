//
//  DeviceCommunicatingFunction.swift
//  GeotabDriveSDK
//
//  Created by Yunfeng Liu on 2020-01-09.
//

import WebKit

struct DeviceCommunicatingFunctionArgument: Codable {
    let callerId: String
    let error: String? // javascript given error, when js failed providing result, it provides error
    let result: GoDeviceState?
}

class DeviceCommunicatingFunction: ModuleFunction {
    private let module: StateModule
    var callbacks: [String: CallbackWithType<Bool>] = [:]
    init(module: StateModule) {
        self.module = module
        super.init(module: module, name: "deviceCommunicating")
    }
    ...
    override func handleJavascriptCall(argument: Any?, jsCallback: @escaping (Result<String, Error>) -> Void) {
        ...
        callback(Result.success(state.deviceCommunicating))
        callbacks[arg.callerId] = nil
