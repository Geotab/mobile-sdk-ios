...
class GetDutyStatusLogFunction: ModuleFunction {
    private static let functionName: String = "getDutyStatusLog"
    private weak var scriptGateway: (any ScriptGateway)?
    
    var userName = ""
    var callbacks: [String: (Result<String, any Error>) -> Void] = [:]
    
    init(module: Module, scriptGateway: any ScriptGateway) {
    ...
    override func handleJavascriptCall(argument: Any?, jsCallback: @escaping (Result<String, any Error>) -> Void) {
        ...
        guard let result = arg.result else {
            callback(Result.failure(GeotabDriveErrors.JsIssuedError(error: DutyStatusLogError.noDutyStatusLogReturned.rawValue)))
            jsCallback(Result.failure(GeotabDriveErrors.JsIssuedError(error: DutyStatusLogError.noDutyStatusLogReturned.rawValue)))
            callbacks[arg.callerId] = nil
            return
        }
    ...
    func call(_ callback: @escaping (Result<String, any Error>) -> Void) {
        ...
        let script = apiCallScript(templateRepo: Module.templateRepo, template: "ModuleFunction.GetDutyStatusLogFunction.Api", scriptData: ["moduleName": moduleName, "functionName": name, "callerId": callerId, "userName": userName])
