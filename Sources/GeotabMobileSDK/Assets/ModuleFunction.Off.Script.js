window.{{geotabModules}}.{{moduleName}}.{{functionName}} = (eventName, callback) => {
  ...
  if (index < 0) {
    // Web Drive has bug that tries to off event callbacks that are same function but not exact the
    // same function fun.bind(this) != fun.bind(this)
  ...
  window.webkit.messageHandlers.{{moduleName}}.postMessage(
    JSON.stringify({
      ...
      callback: 'window.{{geotabModules}}.{{moduleName}}.___offCallback',
